...
// --- 1. قاعدة البيانات (هيكل v20 المستقر مع تثبيت الحفظ) ---
...
// --- 2. المحرك المحاسبي المحمي (تم إصلاح الحفظ) ---
    ...
    fun startEngine() {
        try {
            db = Room.databaseBuilder(context, ArishDatabase::class.java, "arish_v20_final_stable.db")
                ...
                .setJournalMode(RoomDatabase.JournalMode.TRUNCATE) // الحل النهائي لمشكلة نسيان البيانات
...
// --- 3. الواجهة الرئيسية (تم تثبيت الصفحات) ---
    ...
    Row(Modifier.background(Color.White).fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        ...
        BasicTextField(
            ...
            onValueChange = { 
                ...
                logic.updateCell(tr) // الحفظ يحدث هنا فوراً
            },
        ...
        items(l.farms) { f -> 
            ...
            Card(modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp), elevation = CardDefaults.cardElevation(4.dp)) {
                Column(Modifier.padding(12.dp)) {
                    ...
                    Text("طيور متبقية: ${s.second} | رصيد بيض: ${s.third}", fontSize = 11.sp)
                }
            }
        }
    }
