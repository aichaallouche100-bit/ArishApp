...
interface TransactionDao {
    ...
    @Update
    fun updateSync(tr: Transaction)
    @Delete
    fun deleteSync(tr: Transaction)
}

// 3. مدير قاعدة البيانات (Singleton - الطريقة الرسمية)
    ...
    companion object {
        ...
        fun getDatabase(context: Context): ArishDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(context.applicationContext, ArishDatabase::class.java, "arish_v25_fixed.db")
                    .allowMainThreadQueries() // لضمان الحفظ اللحظي على تابلت v60
...
// 4. المحرك الإداري (ViewModel - لضمان بقاء الداتا)
class ArishViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun load() {
        transactions.clear()
        transactions.addAll(dao.getAllSync())
    }

    fun addRow() {
        val newTr = Transaction(id = System.currentTimeMillis(), farm = farms[0], category = "علف", qty = 0.0, price = 19.375)
        dao.insertSync(newTr) // حفظ فوري في الذاكرة الدائمة
        transactions.add(0, newTr)
    }

    fun updateRow(tr: Transaction) { dao.updateSync(tr) }
    ...
    fun getStats(f: String): Triple<Double, Int, Double> {
        ...
        val exp = m.sumOf { if(it.isExpense) it.qty * it.price else 0.0 }
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("نهر اسطوان المحاسبي", color = Color.White, fontWeight = FontWeight.Bold) },
...
fun LiveGrid(vm: ArishViewModel) {
    Column(Modifier.padding(4.dp)) {
        Button(onClick = { vm.addRow() }, modifier = Modifier.fillMaxWidth().height(45.dp), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF2E7D32))) {
            ...
            Card(Modifier.fillMaxWidth().padding(vertical = 4.dp), elevation = CardDefaults.cardElevation(4.dp)) {
                Column(Modifier.padding(16.dp)) {
                    Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) {
                        Text(farm, fontWeight = FontWeight.Bold); Text("${String.format("%.1f", s.first)} $", color = if(s.first >= 0) Color(0xFF2E7D32) else Color.Red)
                    }
                    Row(Modifier.fillMaxWidth().padding(top = 8.dp), Arrangement.SpaceEvenly) {
                        Text("طيور: ${s.second}", fontSize = 11.sp); Text("بيض: ${s.third}", fontSize = 11.sp)
                    }
                }
            }
        }
...
@Composable fun InventoryScreen(vm: ArishViewModel) {
    Column(Modifier.padding(20.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        Card(Modifier.fillMaxWidth().padding(top = 20.dp), colors = CardDefaults.cardColors(containerColor = Color(0xFFE65100))) {
