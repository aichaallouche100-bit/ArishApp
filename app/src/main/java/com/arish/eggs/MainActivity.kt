...
// 1. هيكل بيانات مبسط جداً لضمان السرعة وعدم الضياع
@Entity(tableName = "arish_final_data")
data class Transaction(
    @PrimaryKey val id: Long, // نستخدم الوقت كعنوان فريد لضمان عدم الانهيار
    var farm: String,
    var category: String,
    var qty: Double,
    var price: Double
...
@Dao interface TransactionDao {
    @Query("SELECT * FROM arish_final_data ORDER BY id DESC")
...
// 2. المحرك المحاسبي المباشر (Direct Engine)
class ArishLogic(context: Context) {
    private val db = Room.databaseBuilder(context, ArishDatabase::class.java, "arish_v18.db")
        .allowMainThreadQueries() // ضروري جداً لسرعة الحفظ في تابلت v60
        ...
        bottomBar = {
            NavigationBar(containerColor = Color(0xFF0D47A1)) {
                ...
                menu.forEachIndexed { i, label ->
                    NavigationBarItem(selected = tab == i, onClick = { tab = i }, icon = { Icon(icons[i], null, tint = if(tab==i) Color.Yellow else Color.White) }, label = { Text(label, color = Color.White) })
                }
            }
        }
    ) { p ->
...
fun LiveGrid(logic: ArishLogic) {
    Column(Modifier.padding(4.dp)) {
        Button(onClick = { logic.addRow() }, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF2E7D32))) {
            Icon(Icons.Default.Add, null); Text(" إضافة سطر جديد")
        }
        Spacer(Modifier.height(4.dp))
        Row(Modifier.background(Color(0xFF455A64)).padding(8.dp).fillMaxWidth()) {
            Text("المزرعة", Modifier.weight(1.2f), color = Color.White)
            Text("الصنف", Modifier.weight(1f), color = Color.White)
            Text("الكمية", Modifier.weight(0.6f), color = Color.White)
            Text("القيمة", Modifier.weight(0.8f), color = Color.White)
        }
...
fun EditableRow(tr: Transaction, logic: ArishLogic) {
    Row(Modifier.background(Color.White).fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        ...
        Text(text = String.format("%.1f", tr.qty * tr.price), modifier = Modifier.weight(0.8f).padding(8.dp), color = if (tr.category == "بيض تحميل" || tr.category == "مدخول") Color(0xFF2E7D32) else Color.Red)
        ...
        items(l.farms) { f -> 
            ...
            Card(Modifier.fillMaxWidth().padding(vertical = 4.dp)) {
                Column(Modifier.padding(12.dp)) {
                    Text(f, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold)
    ...
    Column(Modifier.padding(20.dp)) {
        Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color(0xFFE65100))) {
            Column(Modifier.padding(24.dp)) {
                ...
                Text("${String.format("%.2f", l.getSuperStock())} كيس", fontSize = 35.sp, color = Color.White)
            }
        }
    }
}
